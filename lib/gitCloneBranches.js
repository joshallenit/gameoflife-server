...
exports.cloneAll = function (done) {
    cloneOne('https://github.com/joshallenit/gameoflife-entry-node.git', runNode, done);
    cloneOne('https://github.com/joshallenit/gameoflife-entry-ios.git', runIos, done);
};
...
var cloneOne = function (repoUrl, runFunction, done) {
    ...
    var repoDir = __base + 'clones/' + repoUrl.substr(repoUrl.lastIndexOf('/', repoUrl.lastIndexOf('.'))) + '_';
    var masterDir = repoDir + 'master';
    var branches;
    async.series([
            function (callback) {
...
var runNode = function (machine) {
    async.series([
            ...
            function (callback) {
                ...
                var npm = spawn('npm', ['start'], {cwd: machine.dir, env: env});
