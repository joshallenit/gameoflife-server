...
exports.cloneAll = function (done) {
    async.series([
            //function (callback) {
            //    cloneOne('https://github.com/joshallenit/gameoflife-entry-node.git', 3000, runNode, callback);
            //},
            function (callback) {
                cloneOne('https://github.com/joshallenit/gameoflife-entry-ios.git', 4000, runIos, callback);
            },
        ],
        done);
};
 ...
 */
var cloneOne = function (repoUrl, portStart, runFunction, done) {
    ...
    async.series([
            ...
            function (callback) {
                ...
                // use exec because nodegit does not have a way to get remote branches.
            ...
            function (callback) {
                ...
                async.series(checkoutFunctions, callback);
            }
        ],
        done);
};
            ...
            function (callback) {
                ...
                git
                    ...
                    .then(function (repository) {
                        ...
                        // use exec because nodegit checkout does not work "cannot find local ref to branch HEAD"
    ...
    async.series([
            function (callback) {
                var cmd = 'xcodebuild ' +
                    ...
                    'ONLY_ACTIVE_ARCH=NO  ' +
                        // Cannot be a subdirectory of machine.dir or build fails.
                    'CONFIGURATION_BUILD_DIR="'+machine.dir+'APP"';
                exec(cmd, {cwd: machine.dir, maxBuffer: 1024*1024*2}, function (error, stdout, stderr) {
                    if (error) {
                        return callback(errors.toErrorObject(cmd + ' failed ' + error));
                    }
                    console.log('built stdout', stdout);
                    console.log('built stderr', stderr);
                    machine.status += ', installed';
                    callback();
                });
            },
            function (callback) {
                var args = ['launch', machine.dir+'APP/Game of Life - iOS Entry.app', '--args', '--port', machine.port];
                var simulator = spawn('ios-sim', args, {cwd: machine.dir});
                machine.status += ', starting';
                machine.process = simulator;
                simulator.stderr.on('data', function (data) {
                    // simulator outputs regular log messages to stderr
                    machine.stderr += data;
                });
